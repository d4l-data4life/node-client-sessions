...
function computeHmac(opts, iv, ciphertext, duration, createdAt) {
  const hmacAlg = hmacInit(opts.signatureAlgorithm, opts.signatureKey);
...
function decode(opts, content) {
  ...
  // check hmac
  expectedHmac = computeHmac(opts, iv, ciphertext, duration, createdAt);
...
Session.prototype = {
  ...
  setDuration(newDuration, ephemeral = false) {
  ...
  extend() {
    this.setDuration(this.duration, this.opts.cookie.ephemeral);
  },
...
Object.defineProperty(Session.prototype, 'content', {
  ...
  set: function setContent(value) {
    ...
    Object.defineProperty(value, 'extend', {
      enumerable: false,
      value: this.extend.bind(this),
    });
    Object.defineProperty(value, 'getDuration', {
      enumerable: false,
      value: this.getDuration.bind(this),
    });
...
function clientSessionFactory(opts) {
  ...
  if (!(opts.secret || (opts.encryptionKey && opts.signatureKey))) {
    throw new Error(
      'cannot set up sessions without a secret ' +
  ...
  if (!SIGNATURE_ALGORITHMS[sigAlg]) {
    throw new Error(
      'invalid signatureAlgorithm, supported are: ' +
